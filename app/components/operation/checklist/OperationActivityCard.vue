  ...
  >
    <div v-if="!props.editable" class="mb-3 flex items-start justify-between">
      <div class="flex items-center gap-2 text-xs text-gray-500">
        <span class="rounded bg-gray-100 px-1 font-bold">{{
          executorInitials
        }}</span>
    ...
    <p
      class="mb-2 rounded-md bg-slate-100 px-2 py-4 text-md leading-4 text-slate-700"
    >
    ...
    >
      <span class="text-xs font-semibold text-slate-700">Condition</span>
      <div class="grid grid-cols-2 gap-1.5">
        <Button
          size="sm"
          class="h-7 w-24 rounded-md bg-emerald-300 text-emerald-800 hover:bg-emerald-400"
          :class="
        ...
        <Button
          size="sm"
          class="h-7 w-24 rounded-md bg-rose-300 text-rose-800 hover:bg-rose-400"
          :class="
    ...
    <Textarea
      ...
      class="mb-3 min-h-14 rounded-md border border-slate-200 bg-white px-3 py-2 text-xs"
      @update:model-value="emit('update-notes', String($event || ''))"
    />

    <div v-if="props.editable" class="mb-3 space-y-2">
      <template v-if="props.activity.documentationRequired">
        <Button
          type="button"
          variant="outline"
          :disabled="props.isUploading || props.remainingSlots === 0"
          class="h-8 w-full gap-1.5 rounded-md border-slate-200 bg-slate-100 px-3 text-xs text-slate-600 hover:bg-slate-200"
    ...
    <div v-if="props.visibleDocs.length > 0" class="mb-4">
      ...
      <div class="grid grid-cols-3 sm:grid-cols-4 gap-2">
        <button
          ...
          :key="doc.id"
          type="button"
          class="relative aspect-4/3 overflow-hidden rounded border border-gray-200 hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-800"
    ...
    <div v-if="props.editable && props.pendingDocs.length > 0" class="mb-4">
      ...
      <div class="grid grid-cols-4 gap-2">
        <div
          ...
          :key="pendingDoc.id"
          class="relative aspect-4/3 overflow-hidden rounded border border-gray-200"
        >
          <button
            type="button"
            class="absolute inset-0 w-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-800"
    ...
    <div v-if="props.editable" class="flex justify-end">
      <Button
        class="h-8 min-w-24 rounded-md bg-slate-200 px-4 text-xs font-semibold text-slate-900 hover:bg-slate-300"
