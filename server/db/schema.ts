...
export const operationStatusEnum = pgEnum("operationstatus", [
  "Draft",
  "Active",
  "Complete",
  "Cancelled",
]);
export const operationRoleEnum = pgEnum("operationrole", [
  "SUPERVISOR",
  "STAFF",
]);
...
export const operationJobLists = pgTable("operationjoblists", {
  id: serial("id").primaryKey(),
  jobsectionId: integer("jobsectionid")
    .notNull()
    .references(() => jobsection.id),
  operationId: integer("operationid")
    .notNull()
    .references(() => operations.id),
  executedBy: integer("executedby").references(() => users.id),
  jobDescription: text("jobdescription").notNull(),
  documentationRequired: boolean("documentationrequired")
    .notNull()
    .default(false),
...
export const fieldDocumentations = pgTable("fielddocumentations", {
  id: serial("id").primaryKey(),
  joblistId: integer("operationjoblistid")
    .notNull()
    .references(() => operationJobLists.id),
...
export const fieldReports = pgTable("fieldreports", {
  ...
  crewName: varchar("crewname", { length: 255 })
    .notNull()
    .default(""),
  crewSignRequired: boolean("crewsignrequired").notNull().default(false),
...
export const fieldReportNoteDocumentations = pgTable(
  "fieldreportnotedocumentations",
  {
    id: serial("id").primaryKey(),
    noteId: integer("noteid")
      .notNull()
      .references(() => fieldReportNotes.id),
    documentationId: integer("documentationid")
      .notNull()
      .references(() => fieldDocumentations.id),
  },
);
