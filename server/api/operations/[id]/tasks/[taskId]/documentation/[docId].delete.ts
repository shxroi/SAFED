...
import { fieldDocumentations, operationJobLists, operationsEnroll } from '../../../../../../db/schema'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (
      Number.isNaN(operationId) || operationId < 1 ||
      Number.isNaN(taskId) || taskId < 1 ||
      Number.isNaN(docId) || docId < 1
    ) {
      throw createError({ statusCode: 400, message: 'Invalid operation, task, or documentation ID' })
    }

    const [enrollment] = await db
      .select({ id: operationsEnroll.id })
      .from(operationsEnroll)
      .where(and(eq(operationsEnroll.operationId, operationId), eq(operationsEnroll.userId, Number(sessionUser.id))))
      .limit(1)

    if (!enrollment) {
      throw createError({ statusCode: 403, message: 'You are not enrolled in this operation' })
    }

    const [task] = await db
      .select({ id: operationJobLists.id })
      .from(operationJobLists)
      .where(and(eq(operationJobLists.id, taskId), eq(operationJobLists.operationId, operationId)))
      .limit(1)
    ...
    const [documentation] = await db
      ...
      .where(and(eq(fieldDocumentations.id, docId), eq(fieldDocumentations.joblistId, taskId)))
      .limit(1)
    ...
    await db.delete(fieldDocumentations).where(eq(fieldDocumentations.id, documentation.id))
